    ...
    : QComboBox(parent)
{
    ...
    selectItemView->setIconSize(QSize(30,30));
    //设置各项的间隔
    ...
    selectItemView->setFont(QFont("song", 25));
    ...
    popupView->setFont(QFont("song", 25));
    setView(popupView);
    setModel(popupModel);
...
void MutiComboBox::addItem(const QString &text, const QVariant &userData)
{
...
void MutiComboBox::addItem(const QIcon &icon, const QString &text, const QVariant &userData)
{
    QComboBox::addItem(icon,text,userData);
    popupModel->item(findText(text))->setCheckable(true);
}

void MutiComboBox::addItems(const QStringList &texts)
{
...
QStringList MutiComboBox::currentText()
{
    QStringList items;
    for(int row=0;row<popupModel->rowCount();row++)
    {
...
QList<int> MutiComboBox::currentIndex()
{
    QList<int> indexs;
    for(int row=0;row<popupModel->rowCount();row++)
    {
...
bool MutiComboBox::eventFilter(QObject *watched, QEvent *event)
{
    QFrame* frame=this->findChild<QFrame*>();
...
void MutiComboBox::selectItemViewPress(QPoint pos)
{
